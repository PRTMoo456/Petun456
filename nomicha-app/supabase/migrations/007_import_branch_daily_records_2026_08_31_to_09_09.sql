...
-- ช่วงวันที่ 31 ส.ค. 2569 - 9 ก.ย. 2569 รวม 47 รายการ
--
-- หมายเหตุจากต้นฉบับ:
--   * แถววันที่ 30 ส.ค. เป็นยอดตั้งต้นสำหรับคำนวณวันที่ 31 ส.ค.
--     (ช่องยอดคิดเงิน/ยอดรวม/ยอดแก้วว่าง) จึงไม่นำเข้าเป็นยอดขาย
--   * ไฟล์ไม่มีเวลาเข้า-ออกและสต๊อกวัตถุดิบ จึงไม่สร้าง clock_records
--     และจะยก stock_snapshot ล่าสุดที่มีอยู่ก่อนหน้าไว้ให้ ถ้าไม่มีก็เก็บเป็น {}
--     รายการนำเข้าจะมี created_by เป็น NULL เพื่อไม่สร้างค่าปรับลืมลงเวลาจากเวลาที่ไฟล์ไม่มี
...
-- ผลลัพธ์ที่ SQL Editor แสดง: จำนวนทั้งหมดในไฟล์ / เพิ่มใหม่ / มีอยู่แล้วและตรงกัน
select
